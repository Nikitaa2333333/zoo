...
const catRooms = [
  {
    ...
    note: '9 номеров. Идеально для спокойных котиков.',
    litePmsId: '75241',
  },
  {
    title: 'Улучшенный',
    ...
    note: '4 номера. Можно поселить 2-х кошек. +30% к базовой стоимости.',
    litePmsId: '75240',
  },
  {
    title: 'Комфорт',
    ...
    area: '1.12 м²',
    height: '1.3 м',
    includes: 'Мягкая лежанка, видеонаблюдение',
    note: '4 номера. Можно поселить 2-х кошек. +30% к базовой стоимости.',
    litePmsId: '75242',
  }
];
...
const dogRooms = [
  {
    ...
    note: '12 номеров. Для декоративных пород собак весом до 5 кг.',
  },
  {
    title: 'Улучшенный',
    image: dogComfortImg,
    ...
    note: '2 номера. Маленькие породы собак весом до 10 кг.',
  },
  {
    ...
    height: '1.4 м',
    includes: 'Лежанка, выгул 3 раза/день',
    note: '4 номера. Маленькие породы собак весом до 10 кг.',
  },
  {
    ...
    height: '1.4 м',
    includes: 'Лежанка, выгул 3 раза/день',
    note: '4 номера. Средние породы до 20 кг.',
  },
  {
    title: 'Люкс',
    image: dogLuxuryImg,
    ...
    height: '3 м',
    includes: 'Лежанка, выгул 3 раза/день',
    note: '1 номер. Средние породы до 20 кг.',
  }
];
          ...
          >
             ...
             <div className="grid lg:grid-cols-12 gap-8 md:gap-12">
                <div className="lg:col-span-4 space-y-6">
                  <div className={`p-8 md:p-10 rounded-[2.5rem] md:rounded-[3rem] ${selectedCategory === 'cats' ? 'bg-[#99ed36]' : 'bg-[#ff7e27]'} text-[#141414] shadow-2xl relative overflow-hidden`}>
                    <div className="relative z-10">
                      <h1 className="text-3xl md:text-4xl font-black mb-1">{selectedRoom.title}</h1>
                      <p className="text-lg md:text-xl font-bold opacity-70 mb-6 md:mb-8">{selectedRoom.price}</p>
                      
                      <div className="space-y-3">
                        <div className="bg-white/40 backdrop-blur-sm p-4 rounded-2xl flex justify-between font-bold">
                          <span>Площадь</span><span>{selectedRoom.area}</span>
                        </div>
                        <div className="bg-white/40 backdrop-blur-sm p-4 rounded-2xl flex justify-between font-bold">
                          <span>Высота</span><span>{selectedRoom.height}</span>
                        </div>
                      </div>

                       <div className="mt-8 md:mt-10 pt-8 md:pt-10 border-t border-black/10">
                          <div className="text-xs font-bold mb-1 opacity-60 uppercase tracking-wider">Итого за {totalDays} суток</div>
                          <div className="text-4xl md:text-6xl font-black">{totalDays * (selectedRoom.pricePerNight || 0)} ₽</div>
                       </div>
                    </div>
                  </div>
                </div>
                  ...
                  {(selectedCategory === 'dogs' ? dogRooms : catRooms).map((room, idx) => (
                    ...
                    >
                      ...
                      <div className="p-8 flex flex-col flex-1">
                        <h4 className="text-2xl font-black mb-1">{room.title}</h4>
                        <p className="text-3xl font-black text-stone-900 mb-8">{room.price}</p>
                        
                        <div className="space-y-4 mb-8 flex-1">
                          <div className="flex justify-between items-center text-stone-400 font-bold text-sm">
                            <span>Площадь</span>
                            <span className="text-black">{room.area}</span>
                          </div>
                          <div className="flex justify-between items-center text-stone-400 font-bold text-sm">
                            <span>Высота</span>
                            <span className="text-black">{room.height}</span>
                          </div>
                          <div className="pt-4 border-t border-stone-50">
                             <p className="text-stone-500 font-bold text-sm leading-snug">{room.note || room.includes}</p>
                          </div>
                        </div>
                        
                        <button 
                          onClick={() => handleBooking(selectedCategory || 'cats', room)} 
                          className={`w-full py-5 rounded-full font-black text-base transition-all ${selectedCategory === 'dogs' ? 'bg-[#ff7e27] hover:bg-black text-white' : 'bg-[#141414] text-white hover:bg-[#99ed36] hover:text-black'}`}
                        >
                          Забронировать
                        </button>
                      </div>
              ...
              <div className="max-w-7xl mx-auto px-4 md:px-6">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                   {[
                     ...
                     { icon: Heart, title: "Любовь", text: "Профессиональный уход и безграничное внимание 24/7.", color: "bg-[#141414]", textWhite: true }
                   ].map((item, i) => (
                     <div key={i} className={`p-10 rounded-[2.5rem] border border-stone-50 transition-all ${item.textWhite ? 'bg-[#141414] text-white shadow-2xl' : 'bg-white'}`}>
                        <div className={`w-14 h-14 rounded-2xl flex items-center justify-center mb-10 ${item.color} ${item.textWhite ? 'text-black' : 'text-black'}`}>
                           <item.icon size={28} />
                        </div>
                        <h4 className="text-3xl font-black mb-4">{item.title}</h4>
                        <p className={`${item.textWhite ? 'text-stone-400' : 'text-stone-400'} font-bold text-lg leading-relaxed`}>{item.text}</p>
                     </div>
                   ))}
                </div>
              </div>
